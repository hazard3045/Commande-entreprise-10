...
string prendre_photo(){
    ...
    string nomFichier = "photo_" + to_string(nbImpulsions) + "_" + 
                        ...
                        to_string(clk_interne) + ".jpg"; // MODIFICATION 1 : Extension .raw
    
    // Construit la commande libcamera-raw
    // libcamera-raw est utilisé pour la capture des données brutes du capteur
    string commande = "libcamera-jpeg -o " + nomFichier + " --nopreview -t 1"; // MODIFICATION 2 : Commande libcamera-raw
...
int main(){
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        if (!resultat.empty()) {
            ...
            cout << "Nom de fichier attendu : photo_1_1000_50000.jpg" << endl; // MODIFICATION 3 : Vérification .raw
